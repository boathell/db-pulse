...
DB Pulse 不是新闻聚合器，而是一个中国数据库行业认知与行动系统。它面向 CEO、DBA、数据架构师和数据库从业者，把产品发布、技术文档、论文、资本动作、政策与跨平台传播信号收敛为可验证、可复述、可行动的行业主线。首期海外数据库只作为国内事件的比较证据，不独立形成公开主线。
...
- 必须同时维护数据库内核与架构、分布式/云原生/Serverless、实时分析/湖仓/多模数据、稳定性/安全/运维/成本、产品商业化与行业落地、国产生态/资本/政策标准，以及 OLTP、OLAP/HTAP、湖仓实时、图/时序/向量、开源、云托管、私有化信创和关键行业支线。
...
- `data/snapshot/v2.json` 与 `data/reports/source-health.json` 是受审计的版本化数据，不得通过 `.gitignore` 规避冲突；冲突时应恢复最新快照、重跑对应采集或审计流程，并重新生成公开数据。
