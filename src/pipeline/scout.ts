...
export function buildScoutCard(event: EventRow, kind: (typeof kinds)[number]) {
  const base = {
    ...
    why_now: `数据库版本、生产采用、兼容迁移和成本窗口正在同时变化；未来 7 天的官方发布、故障恢复、用户验证与价格信号将决定它是短期噪声还是结构性转折。`,
    counter_signals: `当前证据仍可能偏向发布方叙事；如果独立生产采用、兼容实测、故障恢复或真实成本证据没有出现，应下调判断并停止扩大投入。`,
    horizon: "7-30d",
  ...
  if (kind === "media") {
    return {
      ...
      target_audience: "CEO、DBA、数据架构师和数据库从业者",
      suggested_action:
  ...
  if (kind === "work") {
    return {
      ...
      hypothesis: `如果该变化能被转译为当前组织的查询、稳定性、迁移或成本指标，就有机会从行业信息变成可见的数据库工作杠杆。`,
      target_audience: "DBA、数据架构师、数据库从业者和业务负责人",
      suggested_action:
        "选择一个真实数据库工作负载，写出成功指标、故障演练和停止条件，用最小验证环境完成一次跨职能评审。",
      artifact_idea: "内部机会 brief、黄金负载、故障演练记录和复盘模板",
    };
  }
  if (kind === "learning") {
    return {
      ...
      target_audience: "需要建立数据库技术与产业判断框架的 CEO、架构师和研究者",
      suggested_action:
  ...
  if (kind === "artifact") {
    return {
      ...
      target_audience: "数据库从业者、DBA、研究工程师和开源数据库维护者",
      suggested_action:
  ...
  if (kind === "influence") {
    return {
      ...
      target_audience: "希望建立数据库专业表达与行业连接的数据库从业者和负责人",
      suggested_action:
  ...
  return {
    ...
    hypothesis: `数据库架构、迁移或运维变化可能让过去成本过高的用户问题首次可解。真正机会不在复刻发布，而在找到愿意为稳定性、效率或成本结果付费的窄场景。`,
    target_audience: "有数据库高频痛点且已有预算的 CEO、DBA 与垂直行业团队",
    suggested_action:
      "48 小时内访谈 5 个目标用户，确认现有数据库方案、付费触发点、兼容边界和不可接受风险；只做一个能验证结果的原型。",
    artifact_idea: "数据库机会假设画布、5 份访谈记录、一个结果型原型和继续/停止决策",
  };
}
